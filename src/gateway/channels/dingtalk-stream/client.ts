...
import { GATEWAY_URL, GET_TOKEN_URL, type GraphAPIResponse } from "./constants.js";
...
export type OnEventReceived = (msg: DWClientDownStream) => EventAckData;
...
export class DWClient extends EventEmitter {
	...
	private onEventReceived: OnEventReceived = (_msg: DWClientDownStream) => {
	...
	registerAllEventListener(onEventReceived: (v: DWClientDownStream) => EventAckData) {
		this.onEventReceived = onEventReceived;
		return this;
	}

	registerCallbackListener(eventId: string, callback: (v: DWClientDownStream) => void) {
		if (!eventId || !callback) {
		...
		return new Promise<void>((resolve, reject) => {
			...
			this.socket.on("close", (_err) => {
				...
				if (this.config.autoReconnect && !this.userDisconnect) {
					...
					this.printDebug(`Reconnecting in ${this.reconnectInterval / 1000} seconds...`);
	...
	async connect() {
		try {
			...
			if (this.config.autoReconnect && !this.userDisconnect) {
				this.printDebug(`Connect failed, retrying in ${this.reconnectInterval / 1000} seconds...`);
